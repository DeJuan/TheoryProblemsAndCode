...
def layout(x):
    ...
    wordList = x.split()
    N = len(wordList)
    minScore = (float("inf"), 0, 0)

    DP= {N:(0, None)}
    DPCalculator(DP, 0, wordList, N)

    lineList = fillLineList(DP)
    lines = []
    previousEntry = 0
    for entry in lineList:
        if entry == 0:
          continue
        lines.append(wordList[previousEntry:entry])
        previousEntry = entry
    # We now have the split of the lines as desired. Now we just have to have it print.
    depth = 1
    previousWidths = []
    document = PDF()
    for entry in lines:
      spaceWidth = calcSpaceWidth(entry)
      counter = 0
      previousWidths = []
      for word in entry:
        ...
        if counter == 0:
          X = 0
          previousWidths.append(wordWidth)
          counter+=1
        else:
          X = sum(previousWidths) + counter*spaceWidth
          previousWidths.append(wordWidth)
          counter +=1
        Y = depth*14
        document.write(X,Y, word)
      depth +=1
    return document
        
      

def calcSpaceWidth(lineOfText):
  sum = 0
  for word in lineOfText:
    sum += width(word)
  numerator = 468 - sum
  denominator = len(lineOfText)-1
  if denominator == 0:
    return 0
  return numerator/denominator


def DPCalculator(DP, i, wordList, N):
  if i in DP.keys():
    return DP[i]
  else:
    DP[i] = min([(score(wordList[i:j])+ DPCalculator(DP, j, wordList, N)[0], j) for j in range(i+1, N+1)])
    return DP[i]
  

def fillLineList(minDct):
  lineList = [0]
  try:
    while True:
      if(minDct[lineList[-1]][0] == float("inf")):
         return lineList
      lineList.append(minDct[lineList[-1]][1])
  except:
    return lineList
  
def score(lineOfText):
  return (badness(lineOfText) - goodness(lineOfText))


def goodness(lineOfText):
  if (lineOfText[-1][-1] == "." or lineOfText[-1][-1] == "," or lineOfText[-1][-1] == ";" or lineOfText[-1][-1] == ":"):
    return 10
  else:
    return 0

def badness(lineOfText):
  minimalWidth = minwidth(lineOfText)
  if minimalWidth > 468:
    return float("inf")

  numerator = 468- minimalWidth
  denominator = len(lineOfText)-1 
